...
# Create FastAPI app
app = FastAPI(title="Main Agent API")

# Add CORS middleware for all clients
app.add_middleware(
    CORSMiddleware,
...
# FastAPI endpoints with CORS enabled
@app.get("/")
async def root():
    return {"message": "Main Agent API", "available_agents": list(agent_registry.keys())}

@app.post("/agent/run", response_model=MessageResponse)
async def run_agent(request: MessageRequest):
    result = await agent.run(request.message)
    return MessageResponse(response=result.output)

@app.post("/agent/send", response_model=MessageResponse)
async def send_to_agent(request: AgentMessageRequest):
    response = await send_message_to_agent(None, request.agent_name, request.message)
...
if __name__ == '__main__':
    import asyncio
    # For development, you can still run the agent directly
